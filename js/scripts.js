...
$(document).ready(function() {
    ...
    var ut_word_rotator = function() {
        var ut_rotator_words = ['ZACK LEMAN', 'Entrepreneur',
                'Philomath', 'FULL-STACK WEB DEVELOPER',
                'iOS DEVELOPER', 'PHOTOGRAPHER'
            ],
            counter = 0;
        setInterval(function() {
            $(".banner-text-rotator").fadeOut(function() {
                $(this).html(ut_rotator_words[
                    counter = (counter + 1) %
                    ut_rotator_words.length
                ]).fadeIn();
            });
        }, 4000);
    }
    ...
    if (iOS == false) {
        if (!device.tablet() && !device.mobile()) {
            ...
            $('#skill').bind('inview', function(event, visible) {
                if (visible === true) {
                    var container = $("#skill .page-title h1");
                    container.shuffleLetters();
                }
            });
            ...
            $('#blog').bind('inview', function(event, visible) {
                if (visible === true) {
                    var container = $("#blog .page-title h1");
                    container.shuffleLetters();
                }
            });
            $('#contact').bind('inview', function(event, visible) {
                if (visible === true) {
                    var container = $("#contact .page-title h1");
                    container.shuffleLetters();
                }
            });
        }
    ...
    $(window).scroll(function() {
        ...
        $('#home-slider .slides li').css('background-position',
            scrollAmount);
    });
    ...
    if (!device.mobile()) {
        ...
        var ut_word_rotator = function() {
            var ut_rotator_words = [
                ...
                counter = 0;
            setInterval(function() {
                $(".gallery-text-rotate5").fadeOut(function() {
                    $(this).html(ut_rotator_words[
                        counter = (counter +
                            1) %
                        ut_rotator_words.length
        ...
        var ut_word_rotator = function() {
            var ut_rotator_words = [
                ...
                counter = 0;
            setInterval(function() {
                $(".gallery-text-rotate8").fadeOut(function() {
                    $(this).html(ut_rotator_words[
                        counter = (counter +
                            1) %
                        ut_rotator_words.length
    ...
    function initPieCharts() {
        ...
        chart.each(function() {
            $(this).appear(function() {
                var $this = $(this),
                    chartBarColor = ($this.data(
                        'bar-color')) ? $this.data(
                        'bar-color') :
                    "rgba(255,255,255,.6)",
                    chartBarWidth = ($this.data(
                        'bar-width')) ? ($this.data(
                        'bar-width')) : 40
                if (!$this.hasClass(
                    pieChartLoadedClass)) {
    ...
    $('.project-title a').click(function(event) {
        ...
        if (fileID != null) {
            $('html,body').animate({
                scrollTop: $('#ajax-container').offset()
                    .top - 100
            }, 500);
        }
        $.ajax({
            ...
            $('#ajax-container').addClass(
                'open-container');
            $('#ajax-container').html(data);
            $('.project-slider').flexslider({
                directionNav: false
            });
            $('#ajax-container').removeClass(
                'closed-container');
